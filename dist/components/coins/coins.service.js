...
let CoinsService = class CoinsService {
    constructor(_coinModel, _networkModel, _currencyModel, _coinPriceModel, _feeInfoModel, _stakingPlanModel, _stakingInfoModel, _userStakeInfoModel, _stakeInvestmentInfoModel, _userModel, _walletModel) {
        ...
        this._stakeInvestmentInfoModel = _stakeInvestmentInfoModel;
                ...
                for (const dbCoin of utils_1.dbCoins) {
                    ...
                    }
                    {
                        if (dbCoin?.isStakingAvailable) {
                            const stakingService = new stake_service_1.StakeService(dbCoin?.stakingContractAddress, networkDocument?.rpcUrl, dbCoin?.contractAddress);
    ...
    async updatePriceFromCoinGeckoTerminal(coin) {
        try {
            ...
            const coinPriceInAllCurrencies = currencies.map((currency) => {
            ...
            return await Promise.all(coinPriceInAllCurrencies.map(async (coinPrice) => {
                console.log('updating price for', coin?.name, coinPrice?.currencyName, coinPrice?.price);
    ...
    async updatePriceForAllCurrencies() {
        try {
            ...
            await Promise.all(allCoins?.filter(item => item?.priceFrom === "COINGECKO").map(async (coin) => {
    ...
    async updateStakingInfoForAllCoins(coinIds = []) {
        try {
            const query = {};
            if (coinIds.length) {
            ...
            await Promise.all(allCoins.map(async (coin) => {
                const stakingService = new stake_service_1.StakeService(coin?.stakingContractAddress, coin?.network?.rpcUrl, coin?.contractAddress);
    ...
    async getInvestmentDataForAllUsers(userIds = []) {
        try {
            const query = {};
            if (userIds.length > 0) {
    ...
    async getAllInvestmentDataForInvestmentIds(investmentIds = [], coinId) {
        try {
    ...
    async getAllInvestmentData(startInvestmentIndex = 1) {
        try {
            const coins = await this._coinModel.aggregate([
                {
                    $match: {
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], CoinsService.prototype, "updateStakingInfoForAllCoins", null);
exports.CoinsService = CoinsService = __decorate([
    ...
    __param(5, (0, mongoose_1.InjectModel)(staking_plan_schema_1.StakingPlan.name)),
    __param(6, (0, mongoose_1.InjectModel)(staking_info_schema_1.StakingInfo.name)),
    __param(7, (0, mongoose_1.InjectModel)(user_stake_info_schema_1.UserStakeInfo.name)),
    __param(8, (0, mongoose_1.InjectModel)(stake_investment_info_schema_1.StakeInvestmentInfo.name)),
    __param(9, (0, mongoose_1.InjectModel)(user_schema_1.User.name)),
    __param(10, (0, mongoose_1.InjectModel)(wallet_schema_1.Wallet.name)),
